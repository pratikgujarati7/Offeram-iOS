...
@implementation UsedCouponTableViewCell
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self)
    {
        ...
        self.imageViewBrand = [[AsyncImageView alloc]initWithFrame:CGRectMake(0, 0, 120, 105)];
        ...
        //REUSE
        self.reuseContainer = [[UIView alloc] initWithFrame:CGRectMake(0, 105, self.innerContainer.frame.size.width, 35)];
        self.reuseContainer.backgroundColor = [MySingleton sharedManager].themeGlobalBlueColor;
        [self.innerContainer addSubview:self.reuseContainer];
        
        self.lblIsCouponReused = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, self.reuseContainer.frame.size.width, 35)];
        self.lblIsCouponReused.font = [MySingleton sharedManager].themeFontFourteenSizeBold;
        self.lblIsCouponReused.textColor = [MySingleton sharedManager].themeGlobalWhiteColor;
        self.lblIsCouponReused.textAlignment = NSTextAlignmentCenter;
        self.lblIsCouponReused.numberOfLines = 1;
        self.lblIsCouponReused.layer.masksToBounds = YES;
        self.lblIsCouponReused.text = @"Reactivated";
        [self.reuseContainer addSubview:self.lblIsCouponReused];
        self.lblIsCouponReused.hidden = true;
        
        self.lblReuseAmount = [[UILabel alloc] initWithFrame:CGRectMake(5, 0, self.reuseContainer.frame.size.width - 105, 35)];
        self.lblReuseAmount.font = [MySingleton sharedManager].themeFontFourteenSizeRegular;
        self.lblReuseAmount.textColor = [MySingleton sharedManager].themeGlobalWhiteColor;
        self.lblReuseAmount.textAlignment = NSTextAlignmentCenter;
        self.lblReuseAmount.numberOfLines = 1;
        self.lblReuseAmount.layer.masksToBounds = YES;
        [self.reuseContainer addSubview:self.lblReuseAmount];
        
        self.btnReuse = [[UIButton alloc] initWithFrame:CGRectMake(self.reuseContainer.frame.size.width - 105, 5, 100, 25)];
        self.btnReuse.titleLabel.font = [MySingleton sharedManager].themeFontFourteenSizeBold;
        self.btnReuse.backgroundColor = MySingleton.sharedManager.themeGlobalWhiteColor;
        [self.btnReuse setTitle:@"Reactivate" forState:UIControlStateNormal];
        [self.btnReuse setTitleColor:MySingleton.sharedManager.themeGlobalBlackColor forState:UIControlStateNormal];
        self.btnReuse.clipsToBounds = true;
        self.btnReuse.layer.cornerRadius = 5;
        [self.reuseContainer addSubview:self.btnReuse];
